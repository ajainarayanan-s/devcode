# DevCode V2 Plugin API

> Design proposal. The API shown here is the intended V2 model and is not fully implemented yet.

This document explains how DevCode V2 plugins contribute agents, commands, skills, integrations, providers, and models without importing `@devcode/core`.

The design has four goals:

- Internal and external plugins use the same API.
- Plugin values use generated `@devcode/sdk` types.
...
```ts
import { defineEffectPlugin } from "@devcode/plugin/v2/effect"
...
When the plugin unloads, DevCode rebuilds the agent registry without this transform. The reviewer disappears automatically.
...
return {
  ...
  "aisdk.sdk": Effect.fn(function* (event) {
    // Runs when DevCode needs an AI SDK provider.
  }),

  "aisdk.language": Effect.fn(function* (event) {
    // Runs when DevCode selects a language model implementation.
  }),
}
```
...
DevCode's default distribution uses an opinionated order.
...
When a plugin is added, DevCode invalidates every domain for which it returned a transform.

When a plugin is removed, DevCode removes its hooks and invalidates those domains. Rebuilding from base state automatically removes the plugin's prior mutations.

When a plugin is replaced, DevCode swaps its hooks, preserves the intended plugin order, and invalidates the affected domains.
