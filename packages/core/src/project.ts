...
export interface Interface {
   ...
   *
   * This exists while the old devcode project service and this core project
...
export class Service extends Context.Service<Service, Interface>()("@devcode/ProjectV2") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(path.join(dir, "devcode")).pipe(
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "devcode"), input.id).pipe(Effect.ignore)
    })
