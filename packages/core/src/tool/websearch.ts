...
import { ToolFailure } from "@devcode/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@devcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.DEVCODE_WEBSEARCH_PROVIDER === "exa" || process.env.DEVCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.DEVCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("DEVCODE_EXPERIMENTAL") || truthy("DEVCODE_ENABLE_EXA") || truthy("DEVCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("DEVCODE_ENABLE_PARALLEL") || truthy("DEVCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `devcode/${InstallationVersion}`,
