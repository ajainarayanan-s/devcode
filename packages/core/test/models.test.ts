...
// test/preload.ts pins DEVCODE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.DEVCODE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.DEVCODE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.DEVCODE_MODELS_PATH = undefined
  Flag.DEVCODE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.DEVCODE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.DEVCODE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.DEVCODE_DISABLE_MODELS_FETCH = false
        }),
        () =>
          provided(
            state,
