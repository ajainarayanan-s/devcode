...
import { AbsolutePath, Location, Model, DevCode, Session, Tool } from "@devcode/core/public"
import { tmpdir } from "./fixture/tmpdir"
import { testEffect } from "./lib/effect"

const it = testEffect(DevCode.layer)

describe("public native DevCode API", () => {
  it.effect("exposes only the intentional Session capabilities", () =>
    Effect.gen(function* () {
      const devcode = yield* DevCode.Service

      expect(Object.keys(devcode).sort()).toEqual(["sessions", "tools"])

      expect(Object.keys(devcode.sessions).sort()).toEqual([
        "context",
        "create",
        "events",
        "get",
      ...
      expect(yield* devcode.sessions.list()).toBeArray()
      yield* devcode.tools.register({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* devcode.sessions.create({
            id: sessionID,
            location: Location.Ref.make({ directory: AbsolutePath.make(tmp.path) }),
          })

          yield* devcode.sessions.switchModel({ sessionID, model })

          expect((yield* devcode.sessions.get(sessionID)).model).toEqual(model)
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap(([available, disabled]) =>
        Effect.gen(function* () {
          ...
          yield* devcode.sessions.switchModel({ sessionID: availableID, model: ref({ variant: "default" }) })
          const disabledError = yield* devcode.sessions
            .switchModel({ sessionID: disabledID, model: ref() })
            .pipe(Effect.flip)
          const missingError = yield* devcode.sessions
          ...
          expect((yield* devcode.sessions.get(availableID)).model).toEqual(ref({ variant: "default" }))
          expect((yield* devcode.sessions.get(disabledID)).model).toBeUndefined()
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* devcode.sessions.switchModel({ sessionID, model: selected })

          const error = yield* devcode.sessions
            .switchModel({ sessionID, model: ref({ variant: "unknown" }) })
            .pipe(Effect.flip)

          expect(error).toBeInstanceOf(Session.VariantUnavailableError)
          expect((yield* devcode.sessions.get(sessionID)).model).toEqual(selected)
        }),
      ),
    ),
  )
