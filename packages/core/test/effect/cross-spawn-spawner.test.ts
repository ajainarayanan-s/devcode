...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "devcode-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["DEVCODE_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { DEVCODE_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("DEVCODE_TEST_SHELL=ok")
      }),
    )
