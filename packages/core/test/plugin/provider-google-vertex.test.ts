...
import { GoogleVertexPlugin } from "@devcode/core/plugin/provider/google-vertex"
  ...
  it.effect("ignores OpenAI-compatible providers that are not Google Vertex", () =>
    Effect.gen(function* () {
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.devcode, (provider) => {
          provider.api = {
            type: "aisdk",
            package: "@ai-sdk/openai-compatible",
            url: "https://devcode.ai/zen/v1",
          }
        }),
      )

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.devcode))
