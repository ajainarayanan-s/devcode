...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@devcode/acme")).toBe("@devcode/acme")
    expect(Npm.sanitize("@devcode/acme@1.0.0")).toBe("@devcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/devcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/devcode/acme.git" : spec
