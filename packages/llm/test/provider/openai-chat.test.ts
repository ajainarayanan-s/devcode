    ...
    LLMClient.generate(
      LLM.updateRequest(request, {
        model: Azure.configure({
          baseURL: "https://devcode-test.openai.azure.com/openai/v1/",
          apiKey: "azure-key",
      ...
      Effect.provide(
        dynamicResponse((input) =>
          Effect.gen(function* () {
            const web = yield* HttpClientRequest.toWeb(input.request).pipe(Effect.orDie)
            expect(web.url).toBe("https://devcode-test.openai.azure.com/openai/v1/chat/completions?api-version=v1")
