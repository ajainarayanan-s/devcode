...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-devcode-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-devcode-directory"] ? decode(request.headers["x-devcode-directory"]) : process.cwd())
