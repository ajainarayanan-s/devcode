import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@devcode/plugin/tui"
...
import type { useDevcodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useDevcodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getDevcodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getDevcodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
