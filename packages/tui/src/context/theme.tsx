...
const themeSource: ThemeSource = {
  async discover() {
    ...
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      directories.push(path.join(current, ".devcode"))
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "devcode")
        draft.active = typeof active === "string" ? active : "devcode"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "devcode")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.devcode, store.mode)
    })
