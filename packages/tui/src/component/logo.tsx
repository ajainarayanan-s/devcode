...
import clickSoundPath from "@devcode/ui/audio/bip-bop-02.mp3" with { type: "file" }
...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; leftInk?: RGBA; rightInk?: RGBA; idle?: boolean; sound?: boolean } = {}) {
  ...
  const burst = (x: number, y: number) => {
    ...
    if (!item) return
    if (props.sound) {
      TuiAudio.loadSoundFile(clickSoundPath).then((sound) => {
        if (sound) TuiAudio.play(sound, { volume: 0.5 })
      })
    }
    const t = performance.now()
    const age = t - item.at
        ...
        {(line, index) => (
          <box flexDirection="row" gap={1}>
            <box flexDirection="row">
              {renderLine(line, index(), props.leftInk ?? theme.primary, !!props.leftInk, 0, frame(), dusk(), idleState())}
            </box>
            <box flexDirection="row">
              {renderLine(
