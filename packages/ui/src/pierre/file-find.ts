...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("devcode-find")
  api.delete("devcode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("devcode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("devcode-find", new Highlight(...rest))
    return true
  }
