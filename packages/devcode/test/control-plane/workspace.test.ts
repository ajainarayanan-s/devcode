...
import { EventSequenceTable } from "@devcode/core/event/sql"
...
import { WorkspaceV2 } from "@devcode/core/workspace"
import { WorkspaceTable } from "@devcode/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@devcode/core/ripgrep"

const originalEnv = {
  DEVCODE_AUTH_CONTENT: process.env.DEVCODE_AUTH_CONTENT,
  DEVCODE_EXPERIMENTAL_WORKSPACES: process.env.DEVCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.DEVCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@devcode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.DEVCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=devcode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.DEVCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.DEVCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.DEVCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=devcode-test")
