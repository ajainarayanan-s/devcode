    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.branch).toBe(`devcode/${info.name}`)
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.branch).toBe("devcode/my-feature")
        }),
      { git: true },
    )
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const svc = yield* Worktree.Service
          yield* git(test.directory, ["branch", "devcode/my-feature"])
    ...
    it.instance(
      ...
      () =>
        withCreatedWorktree(undefined, ({ info }) =>
          Effect.gen(function* () {
            expect(info.name).toBeDefined()
            expect(info.branch ?? "").toStartWith("devcode/")
      ...
      () =>
        withCreatedWorktree({ name: "test-workspace" }, ({ info }) =>
          Effect.gen(function* () {
            ...
            expect(info.branch).toBe("devcode/test-workspace")
          }),
        ),
      { git: true },
    )
