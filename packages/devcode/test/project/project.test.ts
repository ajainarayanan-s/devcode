...
import { ProjectDirectories } from "@devcode/core/project/directories"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const devcodeFile = path.join(tmp, ".git", "devcode")
      expect(yield* Effect.promise(() => Bun.file(devcodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "devcode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "devcode")
      const wrongCache = path.join(parentDir, ".git", "devcode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "devcode")
      const cacheB = path.join(bareB, "devcode")
      const wrongCache = path.join(parentDir, ".git", "devcode")
