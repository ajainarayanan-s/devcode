...
import { LocationServiceMap } from "@devcode/core/location-layer"
...
it.instance("plan agent denies edits except .devcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".devcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".devcode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.DEVCODE_TEST_HOME
      process.env.DEVCODE_TEST_HOME = test.directory
