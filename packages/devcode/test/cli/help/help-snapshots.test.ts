...
// terminal sizes. The default devcode tui command is excluded —
// `devcode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `devcode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real devcode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("devcode CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ devcode }) =>
      Effect.gen(function* () {
        const topLevel = yield* devcode.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* devcode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`devcode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`devcode ${argv.join(" ")} --help`)
        }
