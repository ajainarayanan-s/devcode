...
describe("devcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // DEVCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ devcode }) =>
      Effect.gen(function* () {
        const r = yield* devcode.spawn(["mcp", "list"])
        devcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ devcode }) =>
      Effect.gen(function* () {
        const r = yield* devcode.spawn(["providers", "list"])
        devcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ devcode }) =>
      Effect.gen(function* () {
        const r = yield* devcode.spawn(["models"])
        devcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ devcode }) =>
      Effect.gen(function* () {
        const r = yield* devcode.spawn(["agent", "list"])
        devcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh DEVCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ devcode }) =>
      Effect.gen(function* () {
        const r = yield* devcode.spawn(["session", "list"])
        devcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ devcode }) =>
      Effect.gen(function* () {
        const r = yield* devcode.spawn(["stats"])
        devcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under DEVCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ devcode }) =>
      Effect.gen(function* () {
        const r = yield* devcode.spawn(["db", "path"])
        devcode.expectExit(r, 0, "db path")
