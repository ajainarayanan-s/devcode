...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/devcode.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/devcode.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/devcode.jsonc", dir: ".devcode", suggestion: "devcode" },
        expected:
          'Directory ".devcode" in /tmp/devcode.jsonc is not valid. Rename the directory to "devcode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/devcode.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/devcode.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.devcode.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.devcode.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `devcode models` to list available models",
      "Or check your config (devcode.json) provider/model names",
    ].join("\n")
