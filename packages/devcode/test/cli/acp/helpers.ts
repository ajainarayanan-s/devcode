...
export function createAcpClient(input: Pick<CliFixture, "devcode">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.devcode.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "devcode-local-acp", version: "0.1.0" },
      }),
    )
  })
}
