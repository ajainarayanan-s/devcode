  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("DEVCODE_SERVER_PASSWORD", original.envPassword)
        restoreEnv("DEVCODE_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            DEVCODE_SERVER_PASSWORD: input?.password,
            DEVCODE_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  it.live("serves the web UI through the HTTP API app", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        ...
        client: httpClient(
          new Response("<html>devcode</html>", { headers: { "content-type": "text/html" } }),
          (request) => {
            proxiedUrl = request.url
          },
        ),
      ...
      expect(yield* responseText(response)).toBe("<html>devcode</html>")
      expect(proxiedUrl).toBe("https://app.devcode.ai/")
    }),
  )
  ...
  it.live("strips upstream transfer encoding headers from proxied assets", () =>
    Effect.gen(function* () {
      ...
      expect(response.status).toBe(200)
      expect(proxiedUrl).toBe("https://app.devcode.ai/assets/app.js")
            ...
            Layer.succeed(
              ...
              HttpClient.make((request) =>
                Effect.succeed(
                  HttpClientResponse.fromWeb(
                    request,
                    new Response("<html>devcode</html>", {
                      headers: {
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        username: "devcode",
        disableEmbeddedWebUi: true,
        client: httpClient(new Response("<html>devcode</html>", { headers: { "content-type": "text/html" } })),
      }).request(`/?auth_token=${btoa("devcode:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("devcode:secret")}` },
      })
  ...
  it.live("accepts basic auth passwords containing colons for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("devcode:sec:ret")}` },
      })
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "devcode" }).request("/", {
        method: "OPTIONS",
        headers: {
