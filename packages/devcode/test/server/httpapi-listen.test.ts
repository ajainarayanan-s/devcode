...
const original = {
  ...
  envPassword: process.env.DEVCODE_SERVER_PASSWORD,
  envUsername: process.env.DEVCODE_SERVER_USERNAME,
}
const auth = { username: "devcode", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.DEVCODE_SERVER_PASSWORD
  else process.env.DEVCODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.DEVCODE_SERVER_USERNAME
  else process.env.DEVCODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.DEVCODE_SERVER_PASSWORD = auth.password
  Flag.DEVCODE_SERVER_USERNAME = auth.username
  process.env.DEVCODE_SERVER_PASSWORD = auth.password
  process.env.DEVCODE_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.DEVCODE_SERVER_USERNAME = auth.username
  delete process.env.DEVCODE_SERVER_PASSWORD
  process.env.DEVCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-devcode-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-devcode-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-devcode-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.DEVCODE_DISABLE_DEFAULT_PLUGINS
    process.env.DEVCODE_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.DEVCODE_DISABLE_DEFAULT_PLUGINS
      else process.env.DEVCODE_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-devcode-ticket": "1" },
      })
