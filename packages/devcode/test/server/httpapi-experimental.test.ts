...
import { AccountV2 } from "@devcode/core/account"
import { AccountTable } from "@devcode/core/account/sql"
...
function withCreatedWorktree(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      expect(info).toMatchObject({ name, branch: "devcode/api-test" })
      ...
      Effect.gen(function* () {
        ...
        const [consoleState, consoleOrgs, toolList, toolIDs, worktrees, resources] = yield* Effect.all(
          [
            ...
            request(`${ExperimentalPaths.tool}?provider=devcode&model=gpt-5`, directory),
