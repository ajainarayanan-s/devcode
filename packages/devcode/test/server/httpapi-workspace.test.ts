...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-devcode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-devcode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://devcode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-devcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-devcode-workspace")
