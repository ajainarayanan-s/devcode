...
import { LLMEvent, ToolFailure } from "@devcode/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@devcode/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("devcode"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("devcode") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("devcode") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, devcode, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored devcode auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("devcode") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("devcode"),
