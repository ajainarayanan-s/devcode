...
import { SessionV1 } from "@devcode/core/v1/session"
import { FSUtil } from "@devcode/core/fs-util"
import { ModelsDev } from "@devcode/core/models-dev"
import { HttpRecorder } from "@devcode/http-recorder"
import { HttpRecorderInternal } from "@devcode/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@devcode/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@devcode/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.devcode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.DEVCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["devcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("DEVCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("DEVCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["devcode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "devcode-proxy",
    name: "OpenCode proxy",
    providerID: ProviderV2.ID.devcode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["devcode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.DEVCODE_RECORD_CONSOLE_TOKEN && process.env.DEVCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.devcode,
        name: "OpenCode Zen",
        env: ["DEVCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.DEVCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.DEVCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.DEVCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["devcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("DEVCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("DEVCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("DEVCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set DEVCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the devcode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "devcode.json"),
      JSON.stringify({ $schema: "https://devcode.ai/config.json", ...scenario.config(model) }),
    ),
  )
