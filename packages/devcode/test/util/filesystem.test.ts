...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "devcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "devcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "devcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "devcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["devcode.json", "devcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "devcode.json"),
        path.join(project, "devcode.jsonc"),
      ])
    })
  })
