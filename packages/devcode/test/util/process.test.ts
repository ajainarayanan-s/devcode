...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.DEVCODE_TEST ?? "")'), {
      env: {
        DEVCODE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "DEVCODE_TEST_SHELL"], {
      shell: true,
      env: {
        DEVCODE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("DEVCODE_TEST_SHELL=ok")
  })
