...
- Author package entrypoint is `@devcode/plugin/tui`.
...
```json
{
  "$schema": "https://devcode.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/devcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@devcode/plugin/tui`.
- `@devcode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@devcode/plugin/tui"
...
- If `engines.devcode` is absent, no check is performed (backward compatible).
...
- `devcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `devcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.devcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.devcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.devcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.devcode` dir or `DEVCODE_CONFIG_DIR`, OpenCode installs `@devcode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@devcode/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@devcode/plugin/tui`.
...
- `message` is required; `title` defaults to `"devcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `devcode.default`.
...
- Local plugins persist installed themes under the local `.devcode/themes` area near the plugin config source.
...
- `--pure` / `DEVCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.devcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.devcode/plugins/tui-vim.tsx`
- Local smoke config: `.devcode/tui.json`
- Local smoke theme: `.devcode/plugins/smoke-theme.json`
