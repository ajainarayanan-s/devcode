...
Trim `packages/devcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/devcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/devcode`.
- `[x]` `bun typecheck` from `packages/devcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/devcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/devcode`.
- `[x]` `bun dev generate > /tmp/devcode-openapi.json` from `packages/devcode`.
...
- Edit `packages/devcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/devcode-openapi.json` from `packages/devcode`.
- `./packages/sdk/js/script/build.ts` from repo root.
- Inspect generated SDK request param types before deleting each override.
- `bun typecheck` from `packages/devcode`.

### PR 4: Move Path Parameter Patterns Into ID Schemas

- Audit `PathParameterSchemas` and `pathParameterSchema()` in `public.ts`.
- Check source schemas in files like `packages/devcode/src/session/schema.ts`, `packages/devcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/devcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/devcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
