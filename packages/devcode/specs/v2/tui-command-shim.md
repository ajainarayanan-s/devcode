...
Delete `packages/devcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/devcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/devcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/devcode`
- TUI plugin loader tests in `packages/devcode` if runtime plugin API wiring changed
