...
Practical reference for a future `packages/server` split after the devcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/devcode`.
...
Until enough shared service code lives outside `packages/devcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/devcode`

It should not import `packages/devcode` services while `packages/devcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on devcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/devcode` implementation details.
