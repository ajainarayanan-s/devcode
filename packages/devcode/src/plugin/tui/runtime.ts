...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@devcode/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@devcode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@devcode/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@devcode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@devcode/core/v1/config/plugin"
import { createCommandShim } from "@devcode/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@devcode/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".devcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".devcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".devcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.DEVCODE_PURE ? [] : pluginOrigins
    if (Flag.DEVCODE_PURE && pluginOrigins.length) {
    }
