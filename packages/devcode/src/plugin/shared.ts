...
export const DEPRECATED_PLUGIN_PACKAGES = ["devcode-openai-codex-auth", "devcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, devcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(devcodeVersion) || semver.major(devcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.devcode
  if (typeof range !== "string") return
  if (!semver.satisfies(devcodeVersion, range)) {
    throw new Error(`Plugin requires devcode ${range} but running ${devcodeVersion}`)
  }
}
