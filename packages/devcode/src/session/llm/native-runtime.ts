...
import type { LLMClientShape } from "@devcode/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("devcode"))
    return { type: "unsupported", reason: "provider is not openai, devcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @devcode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains devcode-owned. The native runtime only adapts
      // the @devcode/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
