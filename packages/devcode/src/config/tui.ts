...
import { isRecord } from "@devcode/tui/util/record"
...
import { TuiKeybind } from "@devcode/tui/config/keybind"
...
import { TuiConfig } from "@devcode/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@devcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old devcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.devcode`
  // folders between cwd and home, and DEVCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.DEVCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit DEVCODE_TUI_CONFIG override, if set.
  if (Flag.DEVCODE_TUI_CONFIG) {
    const configFile = Flag.DEVCODE_TUI_CONFIG
  ...
  // 4. `.devcode` directories (and DEVCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".devcode") || dir === Flag.DEVCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".devcode") && dir !== Flag.DEVCODE_CONFIG_DIR) continue
