...
const TUI_SCHEMA_URL = "https://devcode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from devcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const devcode = await devcodeFiles(input)
  for (const file of devcode) {
...
async function devcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "devcode"),
    ...(await Filesystem.findUp(["devcode.json", "devcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "devcode"))
  }
  if (Flag.DEVCODE_CONFIG) files.push(Flag.DEVCODE_CONFIG)
