...
const MANAGED_PLIST_DOMAIN = "ai.devcode.managed"
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/devcode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "devcode")
    default:
      return "/etc/devcode"
  }
}
...
export function managedConfigDir() {
  return process.env.DEVCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
