...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall devcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g devcode-ai",
      pnpm: "pnpm uninstall -g devcode-ai",
      bun: "bun remove -g devcode-ai",
      yarn: "yarn global remove devcode-ai",
      brew: "brew uninstall devcode",
      choco: "choco uninstall devcode",
      scoop: "scoop uninstall devcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "devcode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# devcode") || content.includes(".devcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".devcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".devcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".devcode"))
    ) {
      continue
    }
