import type { PermissionV1 } from "@devcode/core/v1/permission"
// CLI entry point for `devcode run`.
//
// Handles three modes:
...
//      devcode server and runs interactive mode against it.
//
...
import { createDevcodeClient, type OpencodeClient, type ToolPart } from "@devcode/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run devcode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running devcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to DEVCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to DEVCODE_SERVER_USERNAME or 'devcode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const sdk = createDevcodeClient({
        baseUrl: "http://devcode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
