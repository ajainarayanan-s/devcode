...
//
// Enable with DEVCODE_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/devcode/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.DEVCODE_DIRECT_TRACE) {
    state = false
    return undefined
  }
