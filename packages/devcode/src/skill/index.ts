...
import { FrontmatterError } from "@devcode/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@devcode/core/util/glob"
...
const DEVCODE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with devcode. The model's intuition for what an
// devcode.json should look like is often wrong, and devcode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch devcode's own config files gives it the
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, DEVCODE_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@devcode/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_DEVCODE_SKILL_NAME] = {
          name: CUSTOMIZE_DEVCODE_SKILL_NAME,
          description: CUSTOMIZE_DEVCODE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_DEVCODE_SKILL_BODY,
        }
