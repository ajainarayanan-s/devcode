This is a plugin to simulate a remote environment locally. Add this to `.devcode/devcode.jsonc`:

```json
  "plugin": ["../packages/devcode/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/devcode/script/run-workspace-server
```
